...
export default function PokerCard({ value, onClick, isSelected, disabled }: PokerCardProps) {
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={`
        w-12 sm:w-16 rounded-lg shadow-xl transition-all duration-300
        ${isSelected 
          ? 'bg-gradient-to-br from-teal-500 to-cyan-600 text-white scale-105 sm:scale-110 ring-2 sm:ring-4 ring-teal-300 shadow-2xl z-10' 
          : isSpecial
        ...
        ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:scale-105 touch-manipulation'}
        flex items-center justify-center font-bold text-xl sm:text-2xl
        border-2 ${isSelected ? 'border-teal-400' : isSpecial ? 'border-slate-500' : 'border-slate-300'}
        transform hover:rotate-1 hover:rotate-0
      `}
      style={{
        height: 'clamp(4.5rem, 6rem, 6rem)',
